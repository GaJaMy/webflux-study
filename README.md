...
현재 프로젝트는 Gradle 멀티 모듈입니다. `mvc-api`에는 Spring MVC 기준 구현이 있고, `webflux-api`에는 같은 실험을 WebFlux 방식으로 구현합니다. `mock-api`와 `mock-webflux-api`는 외부 API 호출 실험을 위한 별도 지연 API 서버입니다.
...
외부 API 호출 비교에서는 mock 서버를 별도 서버로 띄웁니다. `mock-api`는 MVC + `Thread.sleep` 기반이고, `mock-webflux-api`는 WebFlux + `Mono.delay` 기반입니다. 이렇게 해야 MVC/WebFlux 서버가 자기 자신을 호출하며 같은 worker pool을 고갈시키는 self-call 실험이 아니라, 실제 외부 API 대기 상황에 가까워집니다.
...
mock-webflux-api/src/main/java/com/gajamy/mockwebfluxapi
└── delay
```
...
WebFlux 기반 mock 애플리케이션을 로컬에서 실행합니다. 기본 포트는 `8091`입니다.
...
1차 학습 마무리 요약은 `docs/final-summary.md`에 정리합니다.
